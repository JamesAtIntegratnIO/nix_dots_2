...
— only assess networks/devices you own or have written permission to test.

## One command

    pager           bring the link up, open the dashboard in Firefox, and SSH in
    pager web       just open the Virtual Pager dashboard
    pager ssh       just drop into a root shell
...
- **Wi-Fi management AP** — the Pager's `PagerAndChill` network (WPA2), joined
  ...
  This is management-only (see *Networking*). This is the usual path.
- **USB-C tether** — plug into a **USB-A** port on the Pi with a data cable; the
  ...
  172.16.52.0/24. (Needs the raised USB current budget — already set — or the
  port over-currents and it never enumerates.)

Either way, NetworkManager brings it up automatically and `pager` just works.
...
- **Virtual Pager dashboard** (`:1471`) — the full tactile UI in the browser,
  plus a **web terminal** (root shell with no SSH client) and downloads for
  captured handshakes / loot.
- **SSH** — `ssh root@172.16.52.1`; underneath it's OpenWrt-flavored Linux
  (busybox + Pager helpers like `WIFI_MGMT_AP`).
...
- A REST API almost certainly backs the `:1471` UI (the Mark VII had one) but
  Hak5's Pager API docs are "coming soon" — undocumented for now.

## On-device controls

D-pad to move, **A** (green) confirm, **B** (red) back/cancel. First-boot setup
sets a PIN (locks the on-device UI) and the **root password** (used for SSH and
the Virtual Pager).
...
- On the USB path: `lsusb` should show an RTL8153; `dmesg | tail` for
